...
Các engine này có vai trò **mô tả trạng thái thị trường**, không trực tiếp ra quyết định giao dịch.
...
- Biến động (volitality) cao hay thấp 
...
- Target drift: phân phối của biến mục tiêu (return tương lai) thay đổi như thế nào theo thời gian.
...
- Tạo query theo ticker (hiện tại hỗ trợ mapping query cho cổ phiếu Vingroup: VIC, VHM, VPL, VRE)
- Lấy RSS entries
- Lọc trùng
- Lọc theo thời gian
- Match keyword
...
Workflow có thể lặp lại cho đến khi mô hình đạt trạng thái tốt hơn hoặc hết số lần thử tối đa (MAX_RETRIES).
...
$$
Vchange_{t} = \frac{V_t - V_{t-1}}{V_{t-1}}
$$
...
- **RSI > 70**: Thị trường ở trạng thái **quá mua (Overbought)**, giá có thể điều chỉnh giảm.
- **RSI < 30**: Thị trường ở trạng thái **quá bán (Oversold)**, giá có thể phục hồi tăng.
- **RSI ≈ 50**: Thị trường tương đối cân bằng, chưa có xu hướng rõ ràng.
---
...
Trong đó, EMA (Exponential Moving Average) được tính theo công thức:
...
- $N$: số chu kỳ EMA (12 và 26)
- $Price_t$: giá tại thời điểm $t$
